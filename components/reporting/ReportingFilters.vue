<template>
  <div class="space-y-3">
    ...
    <div>
      ...
      <div v-for="(f, idx) in filters" :key="idx" class="flex items-center gap-2 mb-2">
        <select v-model="filters[idx].field.fieldId" class="border rounded px-2 py-1">
        ...
        <select v-model="filters[idx].operator" class="border rounded px-2 py-1" @change="onOperatorChange(idx)">
        ...
        <input v-if="!['is_null','not_null','between','in'].includes(filters[idx].operator)" v-model="filters[idx].value" class="border rounded px-2 py-1" placeholder="value" />
        <input v-if="filters[idx].operator==='between'" v-model="filters[idx].value.start" class="border rounded px-2 py-1" placeholder="start" />
        <input v-if="filters[idx].operator==='between'" v-model="filters[idx].value.end" class="border rounded px-2 py-1" placeholder="end" />
        <input v-if="filters[idx].operator==='in'" v-model="filters[idx].value" class="border rounded px-2 py-1" placeholder="a,b,c" />
        <button class="text-sm text-red-600 underline" @click="remove(idx)">remove</button>
      </div>
