 ...
 * In development mode (NODE_ENV !== 'production'), emails are saved locally instead of being sent via SendGrid
 */
export async function sendReportEmail(data: ReportEmailData): Promise<EmailSendResult> {
    try {
        const isDev = process.env.NODE_ENV !== 'production'

        // In dev mode, save to dev-emails directory instead of sending via SendGrid
        if (isDev) {
            console.log(`[DEV MODE] Saving email locally instead of sending via SendGrid`)
            ...
            // Save email locally to dev-emails directory
            ...
            console.log(`[DEV MODE] Report email saved locally for ${to.join(', ')} with ${attachments.length} attachments`)
            return { success: true }
        }
